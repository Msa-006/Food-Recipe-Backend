...
const RecipeApp = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const backendData = await apiService.getAllFoodItems();
        if (Array.isArray(backendData)) {
          const mappedBackendRecipes = backendData.map(item => ({
            id: "B" + item.id,
            name: item.name,
            category: item.category || "Main Course",
            image: item.imageUrl || "/FoodRecipe/itm/default.jpg",
            description: item.description || "Delicious recipe from our collection.",
          ...
          setBackendRecipes(mappedBackendRecipes);
        }
      } catch (err) {
      ...
      try {
        ...
        console.error("Failed to load API meals:", err);
      }
  ...
  const combinedRecipes = [...backendRecipes, ...localRecipes, ...apiRecipes];
